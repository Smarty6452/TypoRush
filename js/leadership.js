document.addEventListener("DOMContentLoaded", () => {
  ...
  if (leaderboard.length === 0) {
    topScoresContainer.innerHTML =
      "<p>No scores available yet. Be the first to set a record!</p>";
    ...
    leaderboard.forEach((entry, index) => {
      ...
      item.innerHTML = `
                <span class="leaderboard-rank ${rankClass}">${index + 1}. ${
        entry.username
      }</span>
                <span class="leaderboard-score ${rankClass}">${
        entry.score
      } WPM</span>
            `;
    ...
    const userEntry = leaderboard.find(
      (entry) => entry.username === loggedInUser
    );
    if (userEntry) {
      const userRank =
        leaderboard.findIndex((entry) => entry.username === loggedInUser) + 1;
      console.log(
        `User: ${loggedInUser}, Rank: ${userRank}, Score: ${userEntry.score}`
      ); // Debug: Verify user rank
      ...
      userRankContainer.innerHTML =
        "<p>Take a test to join the leaderboard!</p>";
    }
  }
});
